...
call_steps() {
    ...
    python manage.py push_smart_image --image "${PAAS_APP_IMAGE}" --type legacy --dry-run "${PAAS_SKIP_PUSH_SMART_BASE_IMAGE:-False}" --skip-verify "${PAAS_SMART_DOCKER_REGISTRY_SKIP_SSL_VERIFY:-False}"
    python manage.py push_smart_image --image "${PAAS_HEROKU_RUNNER_IMAGE}" --type cnb --dry-run "${PAAS_SKIP_PUSH_SMART_BASE_IMAGE:-False}" --skip-verify "${PAAS_SMART_DOCKER_REGISTRY_SKIP_SSL_VERIFY:-False}"
}
