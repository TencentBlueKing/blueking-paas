...
class BkAppManifestProcessor:
    ...
    def __init__(self, model_deploy: AppModelDeploy, options: Optional[AdvancedOptions] = None):
    ...
    def build_manifest(self, build: Optional[Build] = None, acl_enabled: bool = False) -> Dict:
        ...
        bkapp_res = BkAppResource(**self.model_deploy.revision.json_value)
        ...
        if build:
            if build.image:
                ...
                self._patch_image(bkapp_res, build.image)
            use_cnb = build.artifact_metadata.get("use_cnb", False)

        # TODO: Replace below code with the manifest generated by `paasng.platform.bkapp_model`,
        ...
        if self.options:
            self._update_image_pull_policy(bkapp_res, self.options.image_pull_policy)

        # 更新注解，包含应用基本信息，增强服务，访问控制，镜像凭证等
        self._inject_annotations(
            bkapp_res, self.env.application, self.env, wl_app, use_cnb=use_cnb, acl_enabled=acl_enabled
        )

        # 注入用户自定义变量，与 YAML 中定义的进行合并，优先级：页面填写的 > YAML 中已有的
        bkapp_res.spec.configuration.env = merge_envvars(
            bkapp_res.spec.configuration.env, generate_user_configurations(env=self.env)
        )

        # 注入平台内置环境变量
        bkapp_res.spec.configuration.env = merge_envvars(
            bkapp_res.spec.configuration.env, generate_builtin_configurations(env=self.env)
        )

        # 注入增强服务信息
        addons.inject_to_app_resource(self.env, bkapp_res)

        # 注入挂载信息
        mounts.inject_to_app_resource(self.env, bkapp_res)
    ...
    def _patch_image(self, bkapp_res: BkAppResource, image: Optional[str] = None):
        if bkapp_res.apiVersion == ApiVersion.V1ALPHA2 and bkapp_res.spec.build:
            bkapp_res.spec.build.image = image

        elif bkapp_res.apiVersion == ApiVersion.V1ALPHA1:
            for p in bkapp_res.spec.processes:
                p.image = image

    def _update_image_pull_policy(self, bkapp_res: BkAppResource, pull_policy: str):
        if bkapp_res.apiVersion == ApiVersion.V1ALPHA2 and bkapp_res.spec.build:
            bkapp_res.spec.build.imagePullPolicy = pull_policy
        elif bkapp_res.apiVersion == ApiVersion.V1ALPHA1:
            for proc in bkapp_res.spec.processes:
                proc.imagePullPolicy = pull_policy
    ...
    def _inject_annotations(
        ...
        bkapp_res.metadata.annotations[BKPAAS_DEPLOY_ID_ANNO_KEY] = str(self.model_deploy.pk)

        # inject bkapp basic info
        bkapp_res.metadata.annotations.update(
            {
        ...
        if bkpa_site_id := get_metadata(wl_app).bkpa_site_id:
            bkapp_res.metadata.annotations[PA_SITE_ID_ANNO_KEY] = str(bkpa_site_id)

        # always inject a flag to tell operator that workloads have created the secret
        bkapp_res.metadata.annotations[IMAGE_CREDENTIALS_REF_ANNO_KEY] = "true"

        # inject access control enable info
        if acl_enabled:
            bkapp_res.metadata.annotations[ACCESS_CONTROL_ANNO_KEY] = "true"
