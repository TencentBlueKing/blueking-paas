...
from paas_wl.bk_app.cnative.specs.constants import MODULE_NAME_ANNO_KEY, OVERRIDE_PROC_RES_ANNO_KEY
...
def list_cnative_module_processes_specs(app: Application, environment: str) -> dict[str, list[dict]]:
    ...
    for res in list_mres_by_env(app, environment):
        module_name = res.metadata.annotations[MODULE_NAME_ANNO_KEY]
        specs = [asdict(spec) for spec in gen_cnative_process_specs(res, environment)]

        # Apply override_proc_res from BkAppResource annotations if available
        # The override_proc_res has the highest priority
        specs = _apply_override_proc_res(res, specs)

        module_processes_specs[module_name] = specs
...
def _apply_override_proc_res(res: BkAppResource, specs: list[dict]) -> list[dict]:
    """Apply override_proc_res from BkAppResource annotations to process specs.

    The override_proc_res is configured by admin in backend, and has the highest priority.
    The data is stored in BkAppResource's metadata.annotations with key OVERRIDE_PROC_RES_ANNO_KEY.

    :param res: BkAppResource object from cluster
    :param specs: List of process spec dicts
    :return: Updated list of process spec dicts
    """
    # Get override_proc_res from annotations
    override_config_str = res.metadata.annotations.get(OVERRIDE_PROC_RES_ANNO_KEY, "")
    if not override_config_str:
        return specs

    # The value is generated by json.dumps() in manifest._get_override_proc_res_config,
    # so it's always a valid JSON string (non-empty dict) or empty string
    override_map: Dict[str, dict] = json.loads(override_config_str)

    # Apply override to each process spec
    for spec in specs:
        proc_name = spec.get("name")
        if proc_name and proc_name in override_map:
            override_res = override_map[proc_name]
            # Override limits and requests if provided
            if "limits" in override_res:
                spec["resource_limit"] = override_res["limits"]
            if "requests" in override_res:
                spec["resource_requests"] = override_res["requests"]
            # Recalculate resource_limit_quota based on new limits
            if "resource_limit" in spec:
                spec["resource_limit_quota"] = {
                    "cpu": int(parse_quantity(spec["resource_limit"]["cpu"]) * 1000),
                    "memory": int(parse_quantity(spec["resource_limit"]["memory"]) / (1024 * 1024)),
                }
