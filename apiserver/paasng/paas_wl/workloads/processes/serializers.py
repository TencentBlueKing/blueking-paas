...
from paas_wl.workloads.resource_templates.utils import AddonManager, AppProbeManager
...
class ProcessSerializer(AppEntitySerializer['Process']):
    ...
    def _construct_pod_body_specs(self, process: 'Process') -> Dict:
        ...
        app_probe_mgr = AppProbeManager(app=process.app, process_type=process.type)
        ...
        main_container = {
            ...
            if process.type != "web"
            else readiness_probe or cattr.unstructure(addon_mgr.get_readiness_probe()),
            'livenessProbe': cattr.unstructure(app_probe_mgr.get_liveness_probe()),
            'startupProbe': cattr.unstructure(app_probe_mgr.get_startup_probe()),
        }
