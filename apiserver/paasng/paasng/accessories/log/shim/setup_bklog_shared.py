...
关键取舍:
  ...
  - 索引隔离: 不同租户通过 `name_en` 后缀 `{tenant_id}` 区分到不同 ES 索引
  ...
  - 并发安全: 同名采集项首次并发创建冲突时, 由 client 自动走 `ignore_exists` 幂等路径
"""
...
from paasng.accessories.log.constants import BK_LOG_SHARED_INDEX_VISIBILITY, DEFAULT_LOG_CONFIG_PLACEHOLDER
...
def _upsert_shared_custom_collector_config(module: Module, app_cfg: AppLogCollectorConfig) -> CustomCollectorConfig:
    ...
    同一租户内多应用并发首次部署会竞争同一 `(biz_id, name_en)`, client 内部会
    自动以 `ignore_exists=True` 幂等创建, 把已存在视为成功并返回已有采集项,
    避免部署失败
    """
...
def _build_shared_custom_collector_config(module: Module, app_cfg: AppLogCollectorConfig) -> CustomCollectorConfig:
    """构造共享采集项 `CustomCollectorConfig`

    复用 `to_custom_collector_config` 生成的 ETL / storage 等基础字段, 再把
    `name_en` / `name_zh_cn` 覆盖为租户级共享名, 并附加三项平台级字段。
    """
    ...
    # platform_index_filter 目前仅作为发给日志平台的元数据标记, 不参与运行时过滤;
    # 真正按 app_code 过滤在查询 DSL 里完成, 见 _build_es_search_params 的 termTemplate。
    # 这里的 field 对齐 RESERVED_FIELDS["app_code"] 中 bk-log 方案的事实字段路径, 方便后续排查。
    cfg.platform_index_filter = PlatformIndexFilter(
...
def _build_es_search_params(name_en: str, shared_bk_biz_id: int, message_field: str) -> ElasticSearchParams:
    ...
    共享模式下采集项挂在租户配置的 CMDB 业务下 (业务 ID 而非 space_id),
    因此索引前缀是 `${biz_id}_bklog_`。同租户多应用共用同一份物理索引, 必须通过
    `termTemplate` 把 `__ext.labels.bkapp_paas_bk_tencent_com_code = {{ app_code }}`
    注入查询 DSL, 否则会把同租户其他应用的日志一并带出。字段路径对齐
    `RESERVED_FIELDS["app_code"]` 中 bk-log 方案的事实字段
    """
