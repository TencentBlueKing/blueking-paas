...
def get_all_policy_combination_config(self) -> list[PolicyCombinationConfig]:
    ...
    for tenant_id, policies in grouped_policies.items():
        service_binding_policy = ServiceBindingPolicy.objects.get(service_id=self.service.uuid, tenant_id=tenant_id)
...
def get_tenant_policy_combination_config(self) -> list[PolicyCombinationConfig]:
    precedence_policies = ServiceBindingPrecedencePolicy.objects.filter(tenant_id=self.tenant_id).order_by("-priority")
    service_binding_policy = ServiceBindingPolicy.objects.get(service_id=self.service.uuid, tenant_id=self.tenant_id)
