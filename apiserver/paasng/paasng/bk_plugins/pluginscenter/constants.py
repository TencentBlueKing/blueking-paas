...
class PluginReleaseMethod(StrEnum):
    """插件发布方式"""
...
class PluginBasicInfoAccessMode(StrEnum):
    """插件基本信息查看模式"""
...
class PluginReleaseVersionRule(StrEnum):
    """插件发布版本号规则"""
...
class SemverAutomaticType(StrEnum):
    """语义化版本生成规则"""
...
class ReleaseStageInvokeMethod(StrEnum):
    """发布步骤触发方式"""
...
class MarketInfoStorageType(StrEnum):
    """市场信息存储类型"""
...
class PluginReleaseStatus(StrEnum):
    """插件发布状态"""
...
class PluginReleaseType(StrEnum):
    """插件版本发布类型"""
...
class StatusPollingMethod(StrEnum):
    """发布阶段的状态轮询方式"""
...
class PluginRevisionType(StrEnum):
    """代码版本类型"""
