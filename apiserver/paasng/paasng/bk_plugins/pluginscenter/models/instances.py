...
def is_release_strategy_organization_changed(release: PluginRelease) -> bool:
    ...
    # 获取两个发布策略的组织 ID 集合
    latest_org_ids = (
        {s["id"] for s in latest_release_strategy.organization} if latest_release_strategy.organization else set()
    )
    second_org_ids = (
        {s["id"] for s in second_release_strategy.organization} if second_release_strategy.organization else set()
    )
