...
from paasng.dev_resources.servicehub.manager import SvcAttachmentDoesNotExist, mixed_service_mgr
    ...
    def handle(self, source: str, name: str, region: str, dry_run: bool, *args, **options):
        ...
        for app_code, app_data in to_del_data.items():
            ...
            for module_name, env_list in app_data.items():
                ...
                # 检查如果 2 个环境都都需要解绑，或者已经分配实例的环境都需要解绑，则直接解绑模块
                if (set(env_list) == set(AppEnvName.get_values())) or (set(env_list) == set(svc_provisioned_envs)):
