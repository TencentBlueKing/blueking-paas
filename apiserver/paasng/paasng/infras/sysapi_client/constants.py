...
class ClientAction(StrStructuredEnum):
    ...
    # 目前仅供 AIDev 平台使用，用于为 AI Agent 应用签发用户态 OAuth Token
    FETCH_AI_AGENT_APP_USER_TOKEN = EnumField(
        "sysapi:fetch:ai-agent-app-user-token", label=_("签发 AI Agent 应用的用户态 Token")
    )
