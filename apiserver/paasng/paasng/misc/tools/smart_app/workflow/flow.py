...
from paasng.misc.tools.smart_app.models import SmartBuild, SmartBuildPhase, SmartBuildPhaseType
from paasng.misc.tools.smart_app.models import SmartBuildStep as SmartBuildStepModel
from paasng.misc.tools.smart_app.utils.output import SmartBuildStream, StreamType, get_default_stream
...
class SmartBuildProcedure:
    ...
    def __init__(
        ...
        smart_build: Optional[SmartBuild],
        title: str,
...
class SmartBuildStateMgr:
    ...
    def __init__(
        self, smart_build: SmartBuild, phase_type: SmartBuildPhaseType, stream: Optional[SmartBuildStream] = None
    ):
        self.smart_build = smart_build
...
class SmartBuildCoordinator:
    ...
    def release_lock(self, expected_smart_build: Optional[SmartBuild] = None):
    ...
    def release_if_polling_timed_out(self, expected_smart_build: SmartBuild):
...
class SmartBuildStep:
    """Base class for a smart build step"""

    phase_type: Optional[SmartBuildPhaseType] = None

    def __init__(self, smart_build: SmartBuild, stream: Optional[SmartBuildStream] = None):
        if not self.phase_type:
            raise NotImplementedError("phase type should be specific firstly")

        self.smart_build = smart_build
        self.stream = stream or get_default_stream(smart_build)
        self.state_mgr = SmartBuildStateMgr(smart_build, self.phase_type, self.stream)

        self.phase = self.smart_build.smartbuildphase_set.get(type=self.phase_type)
    ...
    @staticmethod
    def procedures(func):
        """A decorator which update SmartBuild automatically when exception happens"""
        ...
        def decorated(self, *args, **kwargs):
            try:
                ...
                logger.exception(f"A critical error happened during deploy[{smart_build.pk}]")
                # The error message has already been written to stream by SmartBuildProcedure context
