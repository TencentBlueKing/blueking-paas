...
class MresDeploymentsViewSet(GenericViewSet, ApplicationCodeInPathMixin):
    ...
    def create(self, request, code, module_name, environment):
        ...
        try:
            manifest = deploy(env, build_manifest(env, deploy=deployment, credential_refs=credential_refs))
        ...
        AppModelDeployStatusPoller2.start({'deploy_id': deployment.id}, DeployStatusHandler)
