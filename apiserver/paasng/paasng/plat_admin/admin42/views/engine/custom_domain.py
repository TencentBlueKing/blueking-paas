...
class CustomDomainManageView(ApplicationDetailBaseView):
    name = "独立域名"
    template_name = "admin42/configuration/applications/detail/engine/custom_domain.html"

    def get_context_data(self, **kwargs):
        kwargs = super().get_context_data(**kwargs)
        application = self.get_application()
        kwargs["env_choices"] = [{"value": value, "text": text} for value, text in AppEnvName.get_choices()]
        kwargs["modules"] = {module["id"]: module for module in ModuleSLZ(application.modules.all(), many=True).data}
        return kwargs
