...
class ResourceQuotaPlanViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        if ResQuotaPlan.objects.filter(plan_name=data["plan_name"]).exists():
            return Response(
                {"detail": f"资源配额方案名称 '{data['plan_name']}' 已存在，请使用其他名称"},
    ...
    def update(self, request, pk):
        ...
        # 内置方案不允许修改名称
        if plan_obj.is_builtin and data["plan_name"] != plan_obj.plan_name:
            return Response({"detail": "内置资源配额方案不允许修改名称"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def destroy(self, request, pk):
        ...
        if plan_obj.is_builtin:
            return Response({"detail": "默认资源配额方案不允许删除"}, status=status.HTTP_400_BAD_REQUEST)
        plan_obj.delete()
