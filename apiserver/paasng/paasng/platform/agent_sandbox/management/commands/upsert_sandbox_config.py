...
"""Command to upsert the per-app sandbox append config (resource limits, etc.)."""
...
class Command(BaseCommand):
    """Create or update the per-app sandbox append config.

    Apps without a config (or with a config that leaves cpu/memory unset) fall back to
    ...
    help = "Create, update or reset the sandbox append config for an application"
    ...
    def handle(self, app_code, cpu, memory, reset, *args, **options):
        ...
        if reset:
            deleted, _ = SandboxAppendConfig.objects.filter(application=application).delete()
            self.stdout.write(
                self.style.SUCCESS(
                    f"Reset sandbox append config for '{app_code}' "
        ...
        config, created = SandboxAppendConfig.objects.update_or_create(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"{action} sandbox append config for '{app_code}': "
