...
class ApplicationCleaner:
    ...
    def delete_iam_resources(self):
        ...
        delete_builtin_user_groups(app_code=self.application.code, operator=get_system_operator())

        # 删除应用的分级管理员
        logger.info("delete grade manager for application(%s)", self.application)
        delete_grade_manager(app_code=self.application.code, operator=get_system_operator())
