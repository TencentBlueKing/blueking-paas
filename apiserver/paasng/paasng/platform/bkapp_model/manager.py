...
class ProcessServicesManager:
    ...
    def set_auto_created_flag(self, auto_created: bool):
        ...
        """
        ProcessServicesFlag.objects.update_or_create(app_environment=self.env, defaults={"auto_created": auto_created})

    def apply_to(self, model_res: crd.BkAppResource):
        """向 model_res 中注入 process services 配置.
        如果 `auto_created` flag 为 `False`, 表示不需要创建并注入, 否则创建并注入默认配置
        ...
        # TODO 后续处理: 当 auto_created 为 True 时, 创建并注入 process services 配置, 并且设置注解值为 true
        """
        flag, _ = ProcessServicesFlag.objects.get_or_create(app_environment=self.env, defaults={"auto_created": False})
        if not flag.auto_created:
