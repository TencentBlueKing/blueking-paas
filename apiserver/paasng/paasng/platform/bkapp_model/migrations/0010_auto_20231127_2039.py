...
def fix_invalid_module_process_spec_scaling_config_data(apps, schema_editor):
    ...
    for item in qs:
        try:
            ...
            ModuleProcessSpec.objects.using(schema_editor.connection.alias).filter(
                id=item["id"]
            ).update(scaling_config=None)
...
def fix_invalid_env_overlay_scaling_config_data(apps, schema_editor):
    ...
    for item in qs:
        try:
            ...
            ProcessSpecEnvOverlay.objects.using(schema_editor.connection.alias).filter(
                id=item["id"]
            ).update(scaling_config=None)
