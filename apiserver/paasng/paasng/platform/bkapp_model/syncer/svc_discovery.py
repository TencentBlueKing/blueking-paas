...
def sync_svc_discovery(module: Module, svc_disc: SvcDiscConfigSpec) -> CommonSyncResult:
    """Sync svc discovery relations, existing data that is not in the input list may be removed.
    ...
    if not svc_disc.bk_saas:
        ret.deleted_num, _ = SvcDiscConfig.objects.filter(application=module.application).delete()
        return ret
