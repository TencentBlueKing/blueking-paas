...
class DefaultBuildProcessExecutor(DeployStep):
    ...
    def execute(self, metadata: BuildMetadata):
        ...
        # 构建调试: 仅 CNB 构建支持探针机制, Dockerfile/传统 slug-builder 不支持
        build_debug = self.deployment.advanced_options.build_debug and metadata.use_cnb
    ...
    def start_following_logs(self, build_debug: bool = False):
        ...
        :param build_debug: Whether build debug mode is enabled. When True, uses probe-driven
            log streaming instead of Pod-exit-driven.
        """
        if build_debug:
    ...
    def _stream_logs_with_pod_exit(self):
        """正常模式: 日志流跟随直到 Pod 退出, 然后 wait_for_succeeded + clean_slugbuilder"""
        try:
    ...
    def _stream_logs_with_probe_check(self):
        """构建调试模式: 日志流与探针检测并行运行.

        子线程: follow=True 持续拉取日志写入 DeployStream
        主线程: 每 5s 轮询 Pod 探针状态, 探针就绪后关闭日志流
        ...
        try:
            build_result = self._poll_probe_until_ready()
        finally:
            stop_event.set()
    ...
    def _poll_probe_until_ready(self) -> str | None:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "pod_ended":
                logger.info("Builder Pod<%s/%s> has ended, exiting log stream.", ns, name)
                return None
            ...
            if status == "failed":
                ...
                time.sleep(_PROBE_POLL_INTERVAL)
                try:
                    retry_status = handler.check_probe_and_pod(ns, name)
    ...
    def _handle_build_result(self, build_result: str | None):
        ...
        if build_result == "succeeded":
            self.stream.write_message("Build succeeded (probe confirmed).")
        elif build_result == "failed":
            self.stream.write_message(Style.Error("Build failed (probe confirmed). Pod is kept alive for debugging."))
            raise PodNotSucceededError(
                f"Pod<{self.wl_app.namespace}/{self._builder_name}> build failed in debug mode",
                reason="BuildFailed",
                message="Build failed, Pod kept alive for debugging.",
            )
        else:
            # pod_ended: Pod 在探针检测到之前已终止, 按正常流程处理
            self.wait_for_succeeded()
            self.clean_slugbuilder()
