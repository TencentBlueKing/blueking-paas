...
class DefaultBuildProcessExecutor(DeployStep):
    ...
    def _stream_logs_with_probe_check(self):
        ...
        主线程: BuildProbePoller 每 5s 轮询 Pod 探针状态, 探针就绪后关闭日志流
        ...
        poller = BuildProbePoller(self.build_handler, self.wl_app.namespace, self._builder_name)
        try:
    ...
    def _handle_build_result(self, build_result: BuildProbeStatus | None):
        """根据探针结果处理后续流程."""
        if build_result == BuildProbeStatus.SUCCEEDED:
            self.stream.write_message("Build succeeded (probe confirmed).")
        elif build_result == BuildProbeStatus.FAILED:
