...
def on_environment_offline_succeed(
    sender, offline_instance: OfflineOperation, environment: ModuleEnvironment, **kwargs
):
    """如果某个环境被下架，且运营报告存在，需要及时更新运营报告中该环境的评估结果（避免下架后还出现在闲置应用列表中）"""
    report = AppOperationReport.objects.filter(app=environment.application).first()
    if not report:
        return

    # 更新应用评估状态
    mod_name, env_name = environment.module.name, environment.environment
