...
class EncryptedJSONField(BaseEncryptedFieldMixin, serializers.JSONField):
    ...
    :params enable_encrypt_slz_names: 启用解密功能的根 Serializer  类名列表 (如 ["A_SLZ"]), 若不指定则表示全部启用,
    ...
    def __init__(self, **kwargs):
        ...
        self.enable_encrypt_slz_names: list[str] = kwargs.pop("enable_encrypt_slz_names", [])
        super().__init__(**kwargs)
    ...
    def get_parent_slz(self) -> serializers.Serializer:
        ...
        """
        parent = self.parent
        assert parent is not None, "Field must have a parent. Ensure the field is used within a Serializer context"
    ...
    def check_root_slz(self) -> bool:
        ...
        return parent_slz.__class__.__name__ in self.enable_encrypt_slz_names
