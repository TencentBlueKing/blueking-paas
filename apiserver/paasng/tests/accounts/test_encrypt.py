...
class TestPrivateTokenHolder:
    def test_sm4ctr_encrypt(self):
        with connections['default'].cursor() as cursor:
            provider = random_string(10)
            private_token = random_string(10)
            G(PrivateTokenHolder, provider=provider, private_token=private_token)
...
class TestOauth2TokenHolder:
    def test_sm4ctr_encrypt(self):
        with connections['default'].cursor() as cursor:
            provider = random_string(10)
            access_token = random_string(10)
            refresh_token = random_string(10)
            G(Oauth2TokenHolder, provider=provider, access_token=access_token, refresh_token=refresh_token)
