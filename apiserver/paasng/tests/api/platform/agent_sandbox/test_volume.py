...
class TestVolumeShareAPI:
    ...
    def test_share_rejects_unknown_or_self(self, api_client: APIClient, bk_app: Any, volume: Volume) -> None:
        """Grantee must exist, be in the same tenant, and not be the owner."""
        shares_url = reverse(
            "agent_sandbox.volume.shares",
            kwargs={"code": bk_app.code, "volume_id": volume.uuid},
        )
