...
class TestPlan:
    def test_sm4ctr_encrypt(self):
        with connections['default'].cursor() as cursor:
            ...
            G(Plan, config=config, name=name)
...
class TestPreCreatedInstance:
    def test_sm4ctr_encrypt(self):
        with connections['default'].cursor() as cursor:
            credentials = random_string(10)
            uuid = G(PreCreatedInstance, credentials=credentials).uuid
...
class TestServiceInstance:
    def test_sm4ctr_encrypt(self):
        with connections['default'].cursor() as cursor:
            credentials = random_string(10)
            uuid = G(ServiceInstance, credentials=credentials).uuid
