    ...
    def test_protection(self, bk_prod_env, db_roles, forbidden_roles, allowed_roles):
        for role in db_roles:
            EnvRoleProtection.objects.create(
                allowed_role=role, module_env=bk_prod_env, operation=EnvRoleOperation.DEPLOY.value
            )
        ...
        for role in forbidden_roles:
            ...
            # 检查多个角色，其中某个有部署权限的情况
            if db_roles:
                env_role_protection_check(
                    operation=EnvRoleOperation.DEPLOY.value, env=bk_prod_env, roles=[db_roles[0], role]
                )
