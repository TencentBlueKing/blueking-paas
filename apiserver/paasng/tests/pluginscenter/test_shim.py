...
from paasng.pluginscenter.shim import init_plugin_in_view
from paasng.pluginscenter.sourcectl import PluginRepoInitializer
...
class TestInitPlugin:
    ...
    def plugin_repo_initializer(self):
        with mock.patch("paasng.pluginscenter.shim.get_plugin_repo_initializer", spec=PluginRepoInitializer) as mocked:
            yield mocked()

    @pytest.fixture
    def mocked_create_instance(self):
        with mock.patch("paasng.pluginscenter.shim.create_instance") as mocked:
            yield mocked
    ...
    def test_thirdparty_exception(self, plugin, plugin_repo_initializer, mocked_create_instance):
        ...
        with mock.patch("paasng.pluginscenter.shim.add_repo_member"), pytest.raises(APIError):
            init_plugin_in_view(plugin, "")
        assert plugin.repository == "foo"
        assert plugin_repo_initializer.delete_project.called
    ...
    def test_iam_exception(self, plugin, plugin_repo_initializer, mocked_create_instance):
        """测试调用 IAM 接口异常的情景"""
        plugin_repo_initializer.create_project.side_effect = set_plugin_repository("foo")
        with mock.patch("paasng.pluginscenter.shim.add_repo_member"), mock.patch(
            "paasng.pluginscenter.shim.setup_builtin_grade_manager", side_effect=SentinelException
