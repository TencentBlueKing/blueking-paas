...
from paasng.pluginscenter.models import PluginMarketInfo
from paasng.pluginscenter.serializers import PluginMarketInfoSLZ
from paasng.pluginscenter.thirdparty import instance, market
...
def test_instance_upsert_api(mock_client, pd, plugin, handler):
    """测试 instance create/update 接口的序列化"""
    handler(pd, plugin, operator="nobody")
    assert mock_client.call.called is True
    data = mock_client.call.call_args.kwargs["data"]
    # 验证国际化字段存在
...
def test_instance_delete_api(mock_client_session, pd, plugin):
    response = requests.Response()
    response.status_code = 200
    response._content = b"{}"
    mock_client_session.return_value = response
    instance.delete_instance(pd, plugin)
    assert mock_client_session.called
    assert f"delete-instance-{plugin.id}" in mock_client_session.call_args.kwargs["url"]


@pytest.mark.parametrize("handler", (market.create_market_info, market.update_market_info))
def test_market_upsert_api(mock_client, pd, plugin, market_info, handler):
...
def test_market_read_api(mock_client, pd, plugin):
    data = {
        "category": "foo",
