...
class TestToCurlMasked:
    def test_mask_query_params(self):
        test_request = Request('GET', 'https://example.com/api?password=123456&api_key=abcdef')
        prepared_request = test_request.prepare()

        curlify.to_curl(prepared_request)

        assert f'password={quote(MASKED_CONTENT)}' in str(prepared_request.url)
        assert f'api_key={quote(MASKED_CONTENT)}' in str(prepared_request.url)

    def test_mask_json_data(self):
        test_request = Request('POST', 'https://example.com/api', json={"password": "123456", "api_key": "abcdef"})
        prepared_request = test_request.prepare()

        curlify.to_curl(prepared_request)

        json_data = json.loads(prepared_request.body)  # type: ignore
        assert json_data["password"] == MASKED_CONTENT
        assert json_data["api_key"] == MASKED_CONTENT

    def test_mask_form_data(self):
        test_request = Request('POST', 'https://example.com/api', data={"password": "123456", "api_key": "abcdef"})
        prepared_request = test_request.prepare()

        curlify.to_curl(prepared_request)

        form_data = parse_qs(prepared_request.body)  # type: ignore
        assert form_data["password"] == [MASKED_CONTENT]
        assert form_data["api_key"] == [MASKED_CONTENT]

    def test_mask_header_data(self):
        test_request = Request('POST', 'https://example.com/api', headers={"password": "123456", "api_key": "abcdef"})
        prepared_request = test_request.prepare()

        curlify.to_curl(prepared_request)

        assert prepared_request.headers["password"] == MASKED_CONTENT
