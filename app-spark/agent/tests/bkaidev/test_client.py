...
def _router(request: httpx2.Request) -> httpx2.Response:
    ...
    if request.url.path.endswith("/denied"):
        return httpx2.Response(403, json={"error": {"message": "该应用无访问该模型的权限", "code": 40301}})
...
def client() -> AidevApiClient:
    return AidevApiClient(
        ...
        transport=httpx2.MockTransport(_router),
    )
...
async def test_gateway_error_is_typed() -> None:
    async def deny(request: httpx2.Request) -> httpx2.Response:
        return httpx2.Response(403, json={"error": {"message": "该应用无访问该模型的权限", "code": 40301}})

    client = AidevApiClient(
        base_url="https://bkaidev.test/v1",
        access_token="user-token",
        transport=httpx2.MockTransport(deny),
    )
