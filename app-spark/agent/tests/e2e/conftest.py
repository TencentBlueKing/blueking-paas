...
from tests.support.live_fixtures import StartRuntime, conversation_id, runtime, start_runtime, workspace
...
__all__ = ["StartRuntime", "conversation_id", "require_api_key", "runtime", "start_runtime", "workspace"]
...
def require_api_key() -> None:
    ...
    if settings.MODEL.startswith("fake:") or not settings.is_model_ready():
        pytest.skip(
            "e2e needs one complete model mode: MODEL_API_KEY alone, "
            "or BK_AIDEV_ACCESS_TOKEN with MODEL_NAME and MODEL_BASE_URL"
        )
