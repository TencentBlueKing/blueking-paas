...
var _ = Describe("", func() {
	...
	It("Should Update Status.Revision when a bkapp is created", func() {
		...
		Expect(controllerutil.ContainsFinalizer(createdBkApp, v1alpha2.BkAppFinalizerName)).To(BeTrue())
		...
		// Check status
		condAvailable := apimeta.FindStatusCondition(createdBkApp.Status.Conditions, v1alpha2.AppAvailable)
		Expect(condAvailable.Status).To(Equal(metav1.ConditionFalse))

		hookStatus := createdBkApp.Status.FindHookStatus(v1alpha2.HookPreRelease)
		condHooks := apimeta.FindStatusCondition(createdBkApp.Status.Conditions, v1alpha2.HooksFinished)
		...
		Eventually(func() bool {
			...
			condProgressing := apimeta.FindStatusCondition(createdBkApp.Status.Conditions, v1alpha2.AppProgressing)
		...
		By("check the AppAvailable Condition to be True", func() {
			Eventually(func() bool {
				...
				return condAvailable.Status == metav1.ConditionTrue &&
					createdBkApp.Status.Phase == v1alpha2.AppRunning
		...
		Eventually(func() bool {
			...
			cond := apimeta.FindStatusCondition(createdBkApp.Status.Conditions, v1alpha2.AppAvailable)
