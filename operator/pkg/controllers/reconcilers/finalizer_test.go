...
var _ = Describe("Test BkappFinalizer", func() {
		...
		pod = &corev1.Pod{
			...
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					v1alpha2.HookTypeKey:     "pre-release",
				},
	...
	Context("test Reconcile", func() {
		BeforeEach(func() {
			controllerutil.AddFinalizer(bkapp, v1alpha2.BkAppFinalizerName)
		...
		It("test normal", func() {
			...
			Expect(controllerutil.ContainsFinalizer(bkapp, v1alpha2.BkAppFinalizerName)).To(BeFalse())
		})
		...
		It("test be blocked", func() {
			...
			Expect(controllerutil.ContainsFinalizer(bkapp, v1alpha2.BkAppFinalizerName)).To(BeTrue())

			cond := apimeta.FindStatusCondition(bkapp.Status.Conditions, v1alpha2.AppAvailable)
