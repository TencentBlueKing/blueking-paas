...
func Deployment(bkapp *v1alpha2.BkApp, process string) map[string]string {
	labels := map[string]string{
		v1alpha2.BkAppNameKey:   bkapp.Name,
		v1alpha2.ProcessNameKey: process,
	}
	if appInfo, err := applications.GetBkAppInfo(bkapp); err == nil {
		labels[v1alpha2.BkAppRegionKey] = appInfo.Region
		labels[v1alpha2.BkAppCodeKey] = appInfo.AppCode
		labels[v1alpha2.ModuleNameKey] = appInfo.ModuleName
		labels[v1alpha2.EnvironmentKey] = appInfo.Environment
	}
	return labels
}
...
func PodSelector(bkapp *v1alpha2.BkApp, process string) map[string]string {
...
func AppDefault(bkapp *v1alpha2.BkApp) map[string]string {
	labels := map[string]string{
		v1alpha2.BkAppNameKey: bkapp.Name,
	}
	return labels
