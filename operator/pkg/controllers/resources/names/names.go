...
func PreReleaseHook(bkapp *v1alpha2.BkApp) string {
...
func Deployment(bkapp *v1alpha2.BkApp, process string) string {
	return DNSSafeName(bkapp.GetName() + "--" + process)
}

// Service Return the service name for each process
func Service(bkapp *v1alpha2.BkApp, process string) string {
	return DNSSafeName(bkapp.GetName() + "--" + process)
}

// GPA Return the general-pod-autoscaler name for each process
func GPA(bkapp *v1alpha2.BkApp, process string) string {
	return DNSSafeName(bkapp.GetName() + "--" + process)
}
