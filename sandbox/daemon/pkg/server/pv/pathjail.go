...
// JailRoot 计算某个 volume 的 jail 根目录 = cfsRoot/basePath。
// basePath 由 apiserver 计算下发(形如 app/{volume_uuid_hex}), daemon 视为不可逾越的边界。
func JailRoot(cfsRoot, basePath string) string {
	return filepath.Join(cfsRoot, basePath)
}
...
func Resolve(cfsRoot, basePath, relPath string) (full, jailRoot string, err error) {
	jailRoot = JailRoot(cfsRoot, basePath)
...
//
// 它用于挡住用户在 CFS 上创建的、指向 jail 外的 symlink。filepath.EvalSymlinks 要求路径存在,
