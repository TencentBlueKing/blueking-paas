...
export default {
  ...
  '当前主访问入口为：{p1} 进程下的 {p2} 端口。': 'The current main access point is: Port {p2} under process {p1}.',
  '确定切换为：{p1} 进程下的 {p2} 端口？': 'Are you sure to switch to: Port {p2} under process {p1}?',
  '取消后，该模块将不再暴露外部访问地址。': 'After cancellation, this module will no longer expose an external access address.',
  '开启后，应用内部通信可通过“进程服务名称 + 服务端口”访问，通信地址可在“部署管理”页面的进程详情中查看。': 'Once enabled, internal application communication can be accessed via "process service name + service port". The communication address can be viewed in the process details on the "Deployment Management" page.',
  '适合长时间运行的进程，可暴露外部或内部流量': 'Suitable for long-running processes, can be exposed to external or internal traffic',
  '取消访问入口后，才可以关闭进程服务': 'The process service can only be stopped after the access point is canceled',
  '仅 TCP 协议的端口才能设置为访问入口': 'Only ports with the TCP protocol can be set as access points',
  ...
  确认取消访问入口: 'Confirm Cancel Access Point',
  取消访问入口: 'Cancel Access Point',
  设为访问入口: 'Set as Access Point',
  ...
  '访问入口不支持 UDP': 'Access points do not support UDP',
  进程服务说明: 'Process Service Description',
  '访问入口修改后，需重新部署对应环境才能生效。': 'After modifying the access point, the corresponding environment must be redeployed for the changes to take effect.',
  '确认将 {p} 进程的 {s} 端口设置为访问入口': 'Confirm setting port {s} of process {p} as the access point',
  '取消后，该模块将不再提供外部访问地址（如：{r}）。': 'After cancellation, this module will no longer provide an external access address (e.g., {r}).',
  '每个模块可以设置一个访问入口，请求访问地址时（如：{r}）会被转发到访问入口指向的目标服务上。': 'Each module can set an access point. When requesting the access address (e.g., {r}), it will be forwarded to the target service pointed to by the access point.',
};
