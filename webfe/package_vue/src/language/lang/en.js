...
export default {
  ...
  '平台支持将自定义配置文件注入进程内的文件系统，还提供了持久存储功能以实现应用中所有模块和进程的数据共享。': 'The platform supports injecting custom configuration files into the file system within processes, and also provides persistent storage to facilitate data sharing among all modules and processes in the application.',
  ...
  查看计费方式: 'View Billing Method',
  容量: 'Capacity',
  已绑定模块数: 'Number of Bound Modules',
  '来确认删除存储！': 'To confirm the deletion of storage!',
  '容量无法更改，请合理评估容量': 'Capacity cannot be changed, please evaluate capacity reasonably',
  '持久存储会申请对应容量的腾讯云 CFS，新建后就会产生实际的费用，请按需申请。': 'Persistent storage will apply for Tencent Cloud CFS of the corresponding capacity, which will incur actual costs after creation. Please apply as needed.',
  ...
  '删除持久存储后无法恢复，目前有 {c} 个模块绑定了该存储，请确认影响': 'Once deleted, the persistent storage cannot be recovered. Currently, {n} modules are bound to this storage. Please confirm the impact',
