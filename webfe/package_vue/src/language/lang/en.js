...
export default {
  ...
  '建议在代码仓库中的应用描述文件（app_desc.yaml）中定义：': 'It is recommended to define in the application descriptor file (app_desc.yaml) in the code repository:',
  '优先级：如果页面和 app_desc.yaml 同时定义，以 app_desc.yaml 的定义为准。': 'Priority: If both the page and app_desc.yaml are defined, the definition in app_desc.yaml shall prevail.',
  '生效范围：定义后，应用下的所有模块都会生效。': 'Scope: Once defined, all modules under the application will be effective.',
  '多个模块定义：如果多个模块同时定义服务发现，则生效的值将取决于最近部署的模块中的定义。': 'Multiple module definitions: If multiple modules define service discovery simultaneously, the effective value will depend on the definition in the most recently deployed module.',
};
