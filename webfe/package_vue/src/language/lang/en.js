...
export default {
  ...
  '镜像 tag 规则': 'Mirror tag rules',
  '蓝鲸 Buildpack': 'Bk Buildpack',
  'Dockerfile 构建': 'Dockerfile build',
  ...
  新增进程: 'Add process',
  使用示例: 'Usage example',
  '镜像应监听“容器端口“处所指定的端口号，或环境变量值 $PORT 来提供 HTTP服务。': 'The image should listen to the port number specified at the “container port”, or the environment variable value $PORT to provide HTTP service.',
  '接收HTTP请求的端口号．建议镜像直接监听$PORT环境变量不修改本值': 'The port number that receives HTTP requests. It is recommended that the image directly listens to the $PORT environment variable without modifying this value',
  ...
  根据当前负载呵触发条件中设置的阈值自动扩缩容: 'Automatically expand and contract according to the threshold set in the current load and trigger conditions',
