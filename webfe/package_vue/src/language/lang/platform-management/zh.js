...
export default {
  ...
  '在 S-mart 应用中，您在配置页面设置的资源配额将覆盖源码包中的应用描述文件（app_desc.yaml）所定义的配额。修改后，重新部署即可立即生效。': '在 S-mart 应用中，您在配置页面设置的资源配额将覆盖源码包中的应用描述文件（app_desc.yaml）所定义的配额。修改后，重新部署即可立即生效。',
  'Requests 不能大于 Limits': 'Requests 不能大于 Limits',
  显示配置: '显示配置',
  隐藏配置: '隐藏配置',
  云原生应用资源配额: '云原生应用资源配额',
  '停用后不影响移绑定的实例，但不能再绑定到新的应用': '停用后不影响移绑定的实例，但不能再绑定到新的应用',
