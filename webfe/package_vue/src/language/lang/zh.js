...
export default {
  ...
  '当前值由应用描述文件 app_desc.yaml 定义！': '当前值由应用描述文件 app_desc.yaml 定义！',
  '当前值由应用描述文件（app_desc.yaml）定义。如果应用描述文件中的定义没有被删除，在部署时将覆盖您在产品上填写的值。': '当前值由应用描述文件（app_desc.yaml）定义。如果应用描述文件中的定义没有被删除，在部署时将覆盖您在产品上填写的值。',
  '建议在代码仓库中的应用描述文件（app_desc.yaml）中定义：': '建议在代码仓库中的应用描述文件（app_desc.yaml）中定义：',
  '优先级：如果页面和 app_desc.yaml 同时定义，以 app_desc.yaml 的定义为准。': '优先级：如果页面和 app_desc.yaml 同时定义，以 app_desc.yaml 的定义为准。',
  '生效范围：定义后，应用下的所有模块都会生效。': '生效范围：定义后，应用下的所有模块都会生效。',
  '多个模块定义：如果多个模块同时定义服务发现，则生效的值将取决于最近部署的模块中的定义。': '多个模块定义：如果多个模块同时定义服务发现，则生效的值将取决于最近部署的模块中的定义。',
};
