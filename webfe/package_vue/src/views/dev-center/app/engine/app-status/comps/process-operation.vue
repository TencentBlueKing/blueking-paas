<template>
    <div>
        <div>
            <paas-content-loader class="app-container middle" :is-loading="placeholderLoading" placeholder="packages-loading">
    ...
    export default {
        ...
        methods: {
            ...
            watchServerPush () {
                ...
                // 服务异常
                this.cloudServerEvent.onerror = (event) => {
                    // 异常后主动关闭，否则会继续重连
                    console.error(this.$t('推送异常'), event);
                    this.cloudServerEvent.close();
                ...
                // 服务结束
                this.cloudServerEvent.addEventListener('EOF', event => {
            ...
            updateProcessData (data) {
                ...
                if (data.type === 'ADDED') {
                    ...
                    this.allProcesses = this.allProcesses.filter(process => {
                        return process.type !== processData.type;
                    });
                }
            },
            ...
            showProcessDetailDialog (process, index) {
                this.$refs.operateDropRef && this.$refs.operateDropRef.forEach(ref => {
                    ref.close();
                });
                this.processPlan = {
            ...
            async updateProcessConfig () {
                ...
                const processType = this.processPlan.processType;
                const planForm = {
                    process_type: processType,
                    operate_type: 'scale',
            ...
            async updateProcess (process, index) {
                ...
                const targetStatus = process.targetStatus;
                const patchForm = {
                    process_type: processType,
