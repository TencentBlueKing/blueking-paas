    ...
    <div class="port-config">
        ...
        <div class="content" style="position: relative;">
            <table class="ps-table ps-table-border mt20" style="width: 100%;">
                ...
                <tr>
                    <td class="has-right-border td-title"> {{ $t('当前类型') }} </td>
                    <td>{{moduleEntryInfo.type === 1 ? $t('子路径') : $t('子域名')}}</td>
                </tr>
                <tr v-if="rootDomains.length > 1">
                    <td class="has-right-border td-title"> {{ $t('默认根域') }} </td>
                    <td class="root-domains-row" :class="{ 'td-focus': isEdited }" style="">
                        <div class="root-domains-wrapper">
                            <div v-for="(item,index) in rootDomains" :key="index" class="pt10 pb10 root-domain">
                                <bk-radio-group v-model="rootDomainDefault">
                                    <bk-radio :value="item" :disabled="!isEdited">
                        ...
                        <div class="action-box">
                            ...
                            <template v-else>
                                <bk-button
                                    ...
                                    @click.stop.prevent="handleShowDialog"> {{ $t('保存') }} </bk-button>
                                <bk-button
                                    theme="primary"
                                    text
                                    @click.stop.prevent="handleCancel"> {{ $t('取消') }} </bk-button>
                            </template>
                        </div>
                    </td>
                </tr>
