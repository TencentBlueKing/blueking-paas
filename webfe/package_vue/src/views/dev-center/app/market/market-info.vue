    ...
    <section v-show="!isDataLoading">
        <div class="ps-action-header mb20 mt5">
            <div :class="['icon-wrapper', { 'active': appMarketConfig.enabled }]">
                <span :class="['paasng-icon', { 'paasng-lock': !appMarketConfig.enabled, 'paasng-unlock': appMarketConfig.enabled }]"></span>
            </div>
            <div class="title">
        ...
        <template v-if="!appPreparations.all_conditions_matched">
            <div :class="['ps-tip-block', 'is-danger']">
                <p class="title mb10"> {{ $t('无法发布到应用市场') }} </p>
                <section class="content">
                    <p v-for="(condition, index) of appPreparations.failed_conditions" :key="index">
                        <span>{{ $t(condition.message) }}</span>
                        <a href="javascript: void(0);" class="ml10" @click="handleEditCondition(condition)">
            ...
            <div :class="['ps-tip-block', 'is-danger']">
                ...
                <section class="content">
                    <p class="mb10">
                        ...
                        <a href="javascript: void(0);" class="view-risk" @click="viewRisk"> {{ $t('查看风险') }} </a>
                    </p>
                </section>
            </div>
        </template>
        <template v-else-if="appMarketConfig.enabled">
            <div :class="['ps-tip-block', 'is-success']">
                <section class="content">
                    <p>
                        {{ $t('应用已成功发布到蓝鲸应用市场') }}
                        <section class="visit-link" @click="visitAppMarket" v-if="marketAddress"> {{ $t('立即访问') }} </section>
                    </p>
                </section>
            </div>
        </template>

        <span class="ps-span"></span>
                ...
                <bk-form-item
                    ...
                    <bk-radio-group v-model="avaliableAddressValue">
                        <template v-for="(item, index) in avaliableAddress">
                            <div class="moudle-url" :key="index">
                                <bk-radio
                                ...
                                <p class="url" v-if="item.address">{{item.address}}</p>
                            </div>
                        </template>
                    </bk-radio-group>
                </bk-form-item>
        ...
        <template v-else>
            <bk-form
                ...
                <bk-form-item
                    ...
                    :label="$t('访问地址类型：')">
                    <bk-radio :value="3" style="margin-top: 7px;" checked disabled> {{ $t('第三方') }} </bk-radio>
                </bk-form-item>
                ...
                <bk-form-item
                    ...
                    <p class="tip mt10"> {{ $t('用户从桌面打开应用时的访问地址') }} </p>
                </bk-form-item>

                <bk-form-item>
                    <bk-button
                        theme="primary"
                        class="mr10 mt5" :title="$t('保存')"
        ...
        <bk-dialog
            ...
            :width="viewRiskDialog.width" :title="$t('确认风险')"
            @after-leave="afterDialogClose">
            <div>
                <p style="margin-bottom: 10px;"> {{ $t('为了提升开发效率和应用的安全性，蓝鲸开发框架为应用提供了一套完整的用户鉴权体系。') }} </p>
                <p style="margin-bottom: 10px;"> {{ $t('由于当前应用主模块') }} <span style="font-weight: bold;">({{curModuleId}})</span> {{ $t('没有使用蓝鲸开发框架进行初始化，我们无法确认应用是否已经正常接入用户认证与鉴权。') }} </p>
            ...
            <div slot="footer">
                <bk-button theme="primary" :disabled="!riskChecked" @click="goOn"> {{ $t('继续') }} </bk-button>
                <bk-button style="margin-left: 6px;" @click="viewRiskDialog.visiable = false"> {{ $t('取消') }} </bk-button>
            </div>
        </bk-dialog>
        ...
        <bk-dialog
            ...
            <div slot="footer">
                <bk-button theme="primary" :loading="switchAddressDialog.loading" @click="sureSwitchAddress"> {{ $t('确定') }} </bk-button>
                <bk-button theme="default" @click="switchAddressDialog.visiable = false"> {{ $t('取消') }} </bk-button>
            </div>
        </bk-dialog>
    </section>
</template>
