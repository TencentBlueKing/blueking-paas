<template lang="html">
    <div class="right-main">
        ...
        <paas-content-loader :is-loading="loading" placeholder="data-store-loading" :offset-top="30" class="app-container middle">
            <div class="app-container fadeIn">
                <section v-show="!loading">
                    <div class="middle bnone">
                        <h4> {{ $t('已启用的服务') }} </h4>
                        <div v-if="!loading">
                            <ul class="service-list" v-if="serviceListBound.length !== 0">
                                <li v-for="(item, index) in serviceListBound" :key="index">
                                    <div class="service-item">
                                        <router-link v-if="!item.isShare" :to="{ name: 'appServiceInner', params: { id: appCode, service: item.uuid, category_id: item.category.id } }">
                                            <div class="badge">
                                                <div class="logo">
                                        ...
                                        <router-link v-else :to="{ name: 'appServiceInnerShared', params: { id: appCode, service: item.service.uuid, category_id: item.service.category.id } }">
                                            <div class="badge">
                                                <i v-bk-tooltips=" `${$t('共享自')} ${item.ref_module ? item.ref_module.name : ''}${$t('模块')}`"
                                                    class="paasng-icon paasng-info-circle ref-module-icon"></i>
                                                <div class="logo">
                                                    <img :src="item.service.logo" alt="">
                                                </div>
                            ...
                            <div v-if="serviceListBound.length === 0">
                                <div class="ps-no-result multiNotes">
                                    <div><i class="paasng-icon paasng-empty"></i></div>
                    ...
                    <div class="middle bnone service">
                        <h4> {{ $t('未启用的服务') }} </h4>
                        <div v-if="!loading">
                            <ul class="service-list" v-if="serviceListUnbound.length !== 0">
                                <li v-for="(item, index) in serviceListUnbound" :key="index">
                                    <div class="service-item service-item-with-console">
                                        ...
                                        >
                                            ...
                                            <div class="dyna-info">
                                                <i class="paasng-icon paasng-clipboard"></i> {{ $t('简介') }}
                                            </div>
                                        </router-link>
                                    </div>

                                    <div class="service-console">
                                        <a class="ps-btn ps-btn-default ps-btn-disabled" v-bk-tooltips="$t('S-mart应用请在配置文件中设置并开启增强服务')" v-if="isSmartApp">
                                        ...
                                        <a class="ps-btn ps-btn-default" @click="enableService(item)" v-else>
                                            <template v-if="item.specifications.length">
                                                <span> {{ $t('配置并启用服务') }} </span>
                                            </template>
                                        ...
                                        <bk-popconfirm
                                            ...
                                            cancel-text="">
                                            <div slot="content">
                                                <bk-button text theme="primary" @click="handleOpenExportDialog(item, index)"> {{ $t('从其它模块共享服务') }} </bk-button>
                                            </div>
                                            <div class="export-wrapper">
                                                <span class="paasng-icon paasng-down-shape"></span>
                                            </div>
                                        </bk-popconfirm>
                                    </div>
                                </li>
                            ...
                            <div class="" v-if="serviceListUnbound.length === 0">
