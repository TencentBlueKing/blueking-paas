<template>
    <div class="smart-app" data-test-id="createSmart_content_appData">
        ...
        <div v-if="packageData">
            <div class="bk-alert bk-alert-success mb20" data-test-id="createSmart_header_appUpload">
                <i class="paasng-icon paasng-check-circle-shape"></i>
                <div class="bk-alert-content">
                    <div class="bk-alert-title"> {{ $t('源码包上传成功，以下为从app_desc.yml文件中解析出的信息') }} </div>
                    <div class="bk-alert-description"></div>
                </div>
                <div class="bk-alert-close close-text">
                    <bk-button class="f12" :text="true" @click="handleReUpload"> {{ $t('重新上传') }} </bk-button>
                </div>
            </div>
            <div class="package-data-box mb20" data-test-id="createSmart_header_appBaseInfo">
                ...
                <bk-container :col="12" :margin="0">
                    <bk-row class="mb15">
                        ...
                        <bk-col class="pl0 f14" :span="8"><span class="confirm-value">{{packageData.app_description.code}}</span></bk-col>
                    </bk-row>
                    <bk-row>
                        <bk-col class="pr0 f14" :span="2"><span class="confirm-key"> {{ $t('应用名称：') }} </span></bk-col>
                        <bk-col class="pl0 f14" :span="8"><span class="confirm-value">{{packageData.app_description.name}}</span></bk-col>
                    </bk-row>
                </bk-container>
            </div>
            
            <div class="package-data-box mb20" v-for="(packageItem, key) of packageData.app_description.modules" :key="key">
                <p class="package-data-title">{{ key }}<span v-if="packageItem.is_default"> ({{ $t('主') }}) </span></p>
                <bk-container :col="12" :margin="0" data-test-id="createSmart_container_appLanguage">
                    <bk-row class="mb15">
                ...
                <ul class="package-data-services" data-test-id="createSmart_list_appName">
                    <li
        ...
        <div v-else data-test-id="createSmart_btn_appUploader">
            <uploader
                :action="uploadUrl"
                ...
                <a slot="upload-footer" href="https://bk.tencent.com/s-mart" target="_blank" class="f12"> {{ $t('什么是 S-mart 应用？') }} </a>
            </uploader>
        </div>
        <div v-if="isDataLoading" class="data-loading">
            <img src="/static/images/create-app-loading.svg" />
            <p> {{ $t('应用创建中，请稍候') }} </p>
        </div>
        <div v-else class="action-box">
            <bk-button theme="primary" size="large" style="font-size: 14px;" type="submit" @click="handleCommit" :disabled="!packageData"> {{ $t('确认并创建应用') }} </bk-button>
            <bk-button size="large" @click.prevent="back" class="ml15" style="width: 105px; font-size: 14px"> {{ $t('返回') }} </bk-button>
        </div>
    </div>
</template>
